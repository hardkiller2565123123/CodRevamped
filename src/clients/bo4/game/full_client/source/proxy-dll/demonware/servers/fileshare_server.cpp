...
namespace demonware
{
	void fileshare_server::handle(const std::string& packet)
	{
		...
		{
			std::string file = utils::string::split(utils::string::split(packet, '\n')[0], ' ')[1].substr(1, std::string::npos);

			this->send(download_file(file));
		}
		else if (packet.starts_with("PUT"))
		{
			fileshare_upload_file_name = utils::string::split(utils::string::split(packet, '\n')[0], ' ')[1].substr(1, std::string::npos);
		...
		else
		{
			...
			std::string chunk_header = utils::string::split(packet, "\r\n")[0];
	...
	std::string fileshare_server::download_file(const std::string& file)
	{
		...
		if (utils::io::read_file(fileshare::get_file_path(file), &file_buffer))
		{
			...
			http_response.append(utils::string::va("Date: %s\r\n", http_header_time().data()));
			http_response.append(utils::string::va("Content-Length: %d\r\n\r\n", file_buffer.length()));
	...
	{
		if(utils::io::write_file(fileshare::get_file_path(file), std::string(buffer.begin(), buffer.end())))
