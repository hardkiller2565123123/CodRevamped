...
namespace demonware::fileshare
{
	...
	{
		return std::format("http://{}/{}", get_fileshare_host_name(), utils::io::file_name(file));
	}

	std::string get_file_path(const std::string& file)
	{
		return std::format("{}/{}", get_fileshare_directory(), utils::io::file_name(file));
	}

	std::string get_metadata_path(const std::string& file)
	{
		return std::format("{}/{}.metadata", get_fileshare_directory(), utils::io::file_stem(file));
	}
	...
	std::string FileMetadata::SerializeMetaJSON()
	{
		...
		if (this->state == FILE_STATE_DESCRIBED) {
			...
			jsonDocument.AddMember("metadata", utils::cryptography::base64::encode(ddlMetadata), allocator);
		}
	...
	bool FileMetadata::ParseMetaJSON(const std::string& MetaDoc)
	{
		...
		if (this->state == FILE_STATE_DESCRIBED) {
			...
			if (jsonDocument.HasMember("metadata") && jsonDocument["metadata"].IsString())
			{
				...
				this->ddlMetadata = utils::cryptography::base64::decode(metadata_b64);
			}
		}
	...
	{
		std::string json;
		if (utils::io::read_file(path, &json))
		{
	...
	bool FileMetadata::WriteMetaDataJson(const std::string& path, file_state _state)
	{
		...
		return utils::io::write_file(path, this->SerializeMetaJSON());
	}
	...
	std::vector<uint64_t> fileshare_list_demo_ids()
	{
		...
		std::vector<std::string> files = utils::io::list_files(get_fileshare_directory());

		for (auto& file : files)
		{
			std::string rawName = utils::io::file_stem(file);

			if (utils::string::is_integer(rawName)
				&& utils::string::ends_with(file, ".demo")
				&& utils::io::file_exists(get_metadata_path(rawName)))
			{
