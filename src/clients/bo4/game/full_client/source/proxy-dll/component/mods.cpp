...
namespace mods {
	...
	std::filesystem::path mod_dir = storage_paths::Path("custom_data\\t8\\mods");
		...
		public:
			...
			bool read_data_entry(rapidjson::Value& member, const char* mod_name, const std::filesystem::path& mod_path)
			{
				...
				if (!_strcmpi("scriptparsetree", type_val))
				{
					...
					if (!utils::io::read_file(spt_path.string(), &tmp.data))
					{
				...
				else if (!_strcmpi("rawfile", type_val))
				{
					...
					if (!utils::io::read_file(raw_file_path.string(), &tmp.data))
					{
				...
				else if (!_strcmpi("luafile", type_val))
				{
					...
					if (!utils::io::read_file(lua_file_path.string(), &tmp.data))
					{
				...
				else if (!_strcmpi("stringtable", type_val))
				{
					...
					if (!utils::io::read_file(stringtable_file_path.string(), &tmp.data))
					{
			...
			bool load_mods()
			{
				...
				for (const auto& mod : std::filesystem::directory_iterator{ mod_dir })
				{
					...
					if (!utils::io::read_file(filename, &mod_metadata))
					{
	...
	utils::hook::detour db_does_xasset_exist_hook;
	utils::hook::detour scr_gsc_obj_link_hook;
	utils::hook::detour hksl_loadfile_hook;
	...
	utils::hook::detour bg_cache_sync_hook;
