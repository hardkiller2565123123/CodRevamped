...
namespace logger
{
	...
	void write(const int type, std::string str)
	{
		...
		std::ofstream fs;
		fs.open(storage_paths::Path("logs\\t8_client.log"), std::ios_base::app);
	...
	public:
		void pre_start() override
		{
			...
			utils::io::remove_file(storage_paths::Path("logs\\t8_client.log"));
#endif // REMOVE_PREVIOUS_LOG

			write(LOG_TYPE_INFO, "=======================================================================================================");
			write(LOG_TYPE_INFO, " CodRevamped T8 Initializing ... %s[0x%llX]", utils::nt::library{}.get_name().c_str(), utils::nt::library{}.get_ptr());
			write(LOG_TYPE_INFO, "=======================================================================================================");
		}
	};
}
