...
namespace gsc_funcs
{
	...
	namespace
	{
		...
		void shield_to_json(game::scriptInstance_t inst)
		{
			...
			std::string file{ storage_paths::PathA(std::format("custom_data\\t8\\saved\\{}\\{}.json", (inst ? "client" : "server"), fileid).c_str()) };
			...
			utils::io::write_file(file, json_data);
		}
		...
		void shield_from_json(game::scriptInstance_t inst)
		{
			...
			if (!utils::io::read_file(file, &file_content))
			{
	...
	utils::hook::detour scr_get_function_reverse_lookup;
	utils::hook::detour cscr_get_function_reverse_lookup;
	utils::hook::detour scr_get_function;
	utils::hook::detour cscr_get_function;
	utils::hook::detour scr_get_method;
	utils::hook::detour cscr_get_method;
	utils::hook::detour scrvm_error;
	...
	void patch_scrvm_runtime_error()
	{
		...
		void* stub = utils::hook::assemble([scrvm_runtimeerror](utils::hook::assembler& a)
			{
		...
		utils::hook::jump(scrvm_runtimeerror + 0x3A0, stub);
	}
	...
	public:
		void pre_start() override
		{
			...
			enable_dev_func = utils::json_config::ReadBoolean("gsc", "dev_funcs", false);
			// enable custom compiled dev blocks
			enable_dev_blocks = utils::json_config::ReadBoolean("gsc", "dev_blocks", false);
		}
		...
		void post_unpack() override
		{
			...
			utils::hook::jump(0x142890470_g, scrvm_log_compiler_error);

			// better runtime error
			utils::hook::jump(0x142748550_g, get_gsc_export_info);
