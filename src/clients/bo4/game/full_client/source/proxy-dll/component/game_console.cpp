...
namespace game_console
{
	namespace
	{
		...
		void draw_input()
		{
			...
			draw_input_text_and_over("CODREVAMPED T8 >", con_inputWriteDownColor);
			...
			con.output.access([](output_queue& output)
				{
					...
					utils::hook::invoke<bool>(game::Live_SystemInfo, 0, 0, sysinfo_version, 256);
					char sysinfo_livebits[256];
					utils::hook::invoke<bool>(game::Live_SystemInfo, 0, 1, sysinfo_livebits, 256);
					const char* info = utils::string::va("CodRevamped T8, Engine Version: %s, LiveBits: %s", sysinfo_version, sysinfo_livebits);
	...
	bool console_key_event(const int local_client_num, const int key, const bool down, const unsigned int time)
	{
		...
		if (*game::keyCatchers & 1)
		{
			if (down)
			{
				...
				if (key == game::keyNum_t::K_ENTER && !utils::string::is_truely_empty(con.buffer))
				{
					if (history_index != -1)
					{
