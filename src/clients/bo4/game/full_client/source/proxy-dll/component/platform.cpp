...
#include "../../../../../../../shared/common/utils/identity.hpp"
#include "../../../../../../../shared/common/utils/json_config.hpp"
#include "../../../../../../../shared/common/utils/cryptography.hpp"
#include "../../../../../../../../third_party/bo4/winreg/WinReg/WinReg.hpp"
#include "../../../../../../../shared/runtime/StoragePaths.h"
#include "../../../../T8BuildProfile.h"

namespace platform
{
	static std::mutex g_revamped_identity_mutex;
	static std::string g_revamped_username = "Revampedplayer";
	static bool g_revamped_identity_loaded = false;
	...
	{
		std::lock_guard<std::mutex> lock(g_revamped_identity_mutex);
		if (!g_revamped_identity_loaded)
		{
			g_revamped_identity_loaded = true;
			storage_paths::EnsureBaseDirectories();
			const std::string iniPath = storage_paths::GameIniA("t8.ini");
			char value[256]{};
			GetPrivateProfileStringA("Identity", "Username", "Revampedplayer",
				value, static_cast<DWORD>(sizeof(value)), iniPath.c_str());
			g_revamped_username = value[0] ? value : "Revampedplayer";
			if (g_revamped_username.size() > 31) g_revamped_username.resize(31);
		}
		return g_revamped_username.c_str();
	}

	void set_revamped_username(const std::string& name)
	{
		std::lock_guard<std::mutex> lock(g_revamped_identity_mutex);
		g_revamped_username = name.empty() ? "Revampedplayer" : name;
		if (g_revamped_username.size() > 31) g_revamped_username.resize(31);
		g_revamped_identity_loaded = true;
	}
	...
	public:
		...
		{
			// Retail keeps Shield's original Battle.net ownership check. The 2018
			// betas are preservation profiles and should not be killed because a
			// current Battle.net registry key is absent.
