...
#pragma warning(pop)
		...
		void register_hook(const std::string& process, void* stub)
		{
			...
			if (!result)
			{
				...
				logger::write(logger::LOG_TYPE_INFO,
					"[ DEMONWARE ]: beta profile missing optional Winsock import '%s'; continuing", process.c_str());
				return;
			}
	...
	public:
		component()
		{
			...
			{
				// Recovered from the original Aug-2018 beta unlocker.
				tcp_servers.create<auth3_server>("byzantium-pc-beta-auth3.prod.demonware.net");
				tcp_servers.create<lobby_server>("byzantium-pc-beta-lobby.prod.demonware.net");
			}
			else
			{
