...
namespace gsc_custom
{
		...
		for (size_t _inst = 0; _inst < game::SCRIPTINSTANCE_MAX; _inst++)
		{
			...
			for (size_t obj = 0; obj < game::gObjFileInfoCount[inst]; obj++)
			{
				...
				for (size_t i = 0; i < prime_obj->imports_count; i++)
				{
					...
					{
						func = utils::string::va("%s::%s", hashes::lookup_tmp("namespace", imp->name_space), hashes::lookup_tmp("function", imp->name));
					}
	...
	void patch_linking_sys_error()
	{
		...
		// 1C1 = syserr start
		// 19F = end
		utils::hook::jump(scr_get_gsc_obj + 0x1C1, scr_get_gsc_obj + 0x19F);
	}

	utils::hook::detour scr_get_gsc_obj_hook;
	utils::hook::detour gsc_obj_resolve_hook;
