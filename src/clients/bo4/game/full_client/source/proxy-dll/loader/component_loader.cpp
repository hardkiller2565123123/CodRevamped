...
__declspec(allocate(".crcomp$a"))
component_loader::component_factory const g_cr_component_begin = nullptr;

__declspec(allocate(".crcomp$z"))
component_loader::component_factory const g_cr_component_end = nullptr;

void component_loader::instantiate_registered_components()
{
	...
	const auto* current = &g_cr_component_begin + 1;
	const auto* end = &g_cr_component_end;

	for (; current < end; ++current)
	{
		if (*current)
		{
			register_component((*current)());
		}
	}
}
...
bool component_loader::pre_start()
{
	static auto res = []
	{
		instantiate_registered_components();
		clean();
