...
namespace utils::nt
{
	...
	bool library::is_address_in_range(std::uintptr_t address) const
	{
		if (!this->module_)
		{
			return false;
		...
		const auto begin = reinterpret_cast<std::uintptr_t>(this->get_ptr());
		const auto end = begin + static_cast<std::uintptr_t>(header->SizeOfImage);
