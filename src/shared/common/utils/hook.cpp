...
namespace utils::hook
{
	...
	detour::detour()
	{
		// Lazy init: universal proxy objects can be constructed before DllMain.
	}
	...
	{
		MHInit();
		MH_CreateHook(place, target, original);
	}
	...
	void* install(void* target, void* replacement)
	{
		...
		const auto create_status = MH_CreateHook(target, replacement, &trampoline);
	...
	std::optional<std::pair<void*, void*>> iat(const utils::nt::library& library, const std::string& target_library, const std::string& process, void* stub)
	{
		...
		return std::make_optional(std::make_pair(static_cast<void*>(ptr), stub));
	}
